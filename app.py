...
def welcome():
    return (
        ...
        f"/api/v1.0/tobs<br/>"
        f"/api/v1.0/start (please enter as YYYY-MM-DD)<br/>"
        f"/api/v1.0/start/end (please enter as YYYY-MM-DD/YYYY-MM-DD)"
...
def precipitation():
    ...
    print(f"Results for Precipitation - {preci_dict}")
...
def stations():
    session = Session(engine)
    station_que = [Station.station, Station.name, Station.latitude, Station.longitude, Station.elevation]
    queryresult = session.query(*station_que).all()
    session.close()
